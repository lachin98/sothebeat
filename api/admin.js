...
async function getGameStatus(res) {
  const config =
    await sql`SELECT key, value FROM game_config WHERE key IN ('current_phase', 'phases_status', 'online_users')`;
...
async function togglePhase(res, phase) {
  const current =
    await sql`SELECT value FROM game_config WHERE key = 'phases_status'`;
